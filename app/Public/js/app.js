...
$(document).ready(function() {
	...
	$('#refresh').click(function(e) {
		...
		$.SmartMessageBox({
			...
			content : $.widresetMSG || "你确认要重置本地缓存?",
			buttons : '[取消][确认]'
...
$('body').on('click', function(e) {
    ...
    //sort
	$('[role="columnheader"]').click(
			function() {
				_class = $(this).attr('class');
				_sortord = 'asc';
				if (_class.substring('7')
						&& _class.substring('8') != 'desc') {
					_sortord = 'desc';
				}
				location.href = location.pathname + '?field='
						+ $(this).attr('aria-label') + '&sortord='
						+ _sortord;
			});
}); 